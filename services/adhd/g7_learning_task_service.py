...
from models.adhd.g7_learning_task_model import (
    G7LearningTaskAssignRequest, G7AssignedTask,
    G7LearningTaskAssignResponse, G7LearningTaskResult,
    G7LearningTaskResultResponse,
)
...
TASK_DEFINITIONS = {
    "gonogo": {
        ...
        "target": "impulsivity",
        "instructions": {
            1: "dual-rule: කොළ රවුම = ස්පර්ශ, රතු = නොකරන්න. 30 trials, 1.2s.",
            2: "dual-rule: 35 trials, 1.2s. false alarm rate track කරන්න.",
            3: "complex dual-rule stimuli. 40 trials, 1.2s. ඉහළ concentration.",
        },
    },
    "wait_match": {
        "name": "බලා ගැලපීම",
        "target": "impulsivity",
        "instructions": {
            1: "රූපය 1s. 4 options. 15 trials.",
            2: "රූපය 0.8s. 5 options. 18 trials.",
            3: "රූපය 0.5s. 5 options. 20 trials.",
        },
    },
    "audio_sequence": {
        ...
        "target": "inattention",
        "instructions": {
            1: "6 sentence කතාව. 1 play. 5 items. ශ්‍රේෂ්ඨ.",
            2: "6 sentence කතාව. 1 play. 5 items. zero help.",
            3: "6 sentences. 1 play. 5 items. time limit 90s.",
        },
    },
    "spot_change": {
        "name": "වෙනස සොයන්න",
        "target": "inattention",
        "instructions": {
            1: "3 scenes, 3 changes. 5s limit.",
            2: "3 scenes, 3 changes. 4s limit.",
            3: "3 scenes, 3 changes. 3s limit.",
        },
    },
    "attention_grid": {
        "name": "අවධාන ජාලය",
        "target": "maintenance",
        "instructions": {
            1: "5x5 grid. 20s. ~20% targets.",
            2: "6x6 grid. 16s. ~20% targets.",
            3: "6x6 grid. 12s. ~20% targets.",
        },
    },
}
...
def _get_session_number(child_id, grade):
    ...
    return (db["learning_task_results"].count_documents(
        {"child_id":child_id,"grade":grade}) // 3) + 1
...
def g7_assign_tasks(req: G7LearningTaskAssignRequest) -> G7LearningTaskAssignResponse:
    db     = get_db()
    latest = db["adhd_submissions"].find_one(
        {"child_id":req.child_id,"grade":7}, sort=[("created_at",-1)])
    ...
    session_num        = _get_session_number(req.child_id, 7)
    assigned = []
    for tid in task_ids:
        defn = TASK_DEFINITIONS[tid]
        diff = _get_difficulty(req.child_id, tid, 7)
        assigned.append(G7AssignedTask(
            task_id=tid, task_name=defn["name"], difficulty=diff,
            target_deficit=defn["target"], instructions=defn["instructions"][diff],
        ))
    return G7LearningTaskAssignResponse(
        child_id=req.child_id, grade=7, session_number=session_num,
...
def g7_save_task_result(result: G7LearningTaskResult) -> G7LearningTaskResultResponse:
    db    = get_db()
    total = result.total_trials or 1
    score = round((result.correct / total) * 100, 1)
    past  = list(db["learning_task_results"]
                 .find({"child_id":result.child_id,"task_id":result.task_id,"grade":7})
    ...
    db["learning_task_results"].insert_one({
        "child_id":result.child_id,"grade":7,"task_id":result.task_id,
    ...
    return G7LearningTaskResultResponse(ok=True, message="ප්‍රතිඵල සුරකින ලදී",
                                        score_percent=score, next_difficulty=next_diff, encouragement=msg)


def g7_get_progress(child_id):
    db      = get_db()
    results = list(db["learning_task_results"].find({"child_id":child_id,"grade":7})
                   .sort([("created_at",-1),("timestamp",-1)]).limit(20))
    return {"child_id":child_id,"grade":7,"sessions":[_serialize(r) for r in results]}
